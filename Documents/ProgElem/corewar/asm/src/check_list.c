...
** Last update Sun Mar 25 22:50:46 2012 lyoma guillou
*/
...
static int	_is_code(t_node *node, t_header *head)
{
  ...
  if (node->data && my_strlen(node->data) > 2)
...
t_node		*is_valid_format(t_node *node)
{
  ...
  if (tmp->next && !(tmp->next->type & T_ACT) && tmp->next && !(tmp->next->type & T_COD))
    {
      my_puterr("Error: Invalid argument number or invalid instruction on next line");
      tmp = NULL;
    }
  return (tmp);
}
