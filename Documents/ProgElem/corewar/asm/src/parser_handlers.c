/*
** parser_handlers.c for src in /home/czegan_g/work/school/corewar/asm/src
**
** Made by louis duverdier
** Login   <duverd_l@epitech.net>
**
** Started on  Tue Jan 24 16:19:17 2012 louis duverdier
** Last update Sun Mar 25 21:41:39 2012 louis duverdier
*/
...
void		handle_define(t_handler *h, const char *str, int *off)
{
  ...
  *off = 0;
  ...
  if (str[*off] && (is_name || !my_strcmp(name, "comment")))
    {
      while (str[(*off)] && my_is_space(str[*off]))
	++(*off);
      if (is_name && str[*off] == '"')
        _fill_define(h->head->prog_name, str + *off + 1, PROG_NAME_LENGTH);
      else if (str[*off] == '"')
        _fill_define(h->head->comment, str + *off + 1, COMMENT_LENGTH);
    }
