...
** Last update Sun May 20 22:49:48 2012 lyoma guillou
*/
...
static char	*_get_next_pattern(char *str)
{
  ...
  flag.sym = 0;
  flag.tmp = strdup(str);
  while (flag.tmp[i] && (flag.tmp[i] != ' ' || flag.match < 0))
    {
      if (_is_quote(flag.tmp[i]) && (flag.sym == 0 || flag.sym == flag.tmp[i]))
	  {
...
char		**str_to_wordtab(char *src)
{
  ...
  str = strdup(src);
  len = (_count_pattern(str)) ? _count_pattern(str) : 1;
