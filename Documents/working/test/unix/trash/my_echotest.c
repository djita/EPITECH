...
** Last update Sat May 12 16:31:21 2012 lyoma guillou
*/
...
static char	_get_symbol(char *src)
{
  ...
  i = 0;
  if (src[0] == '0')
    return ((char) my_getnbr_base(src + 1, "012345678"));
  if (src[0] == 'x')
    return ((char) my_getnbr_base(src + 1, "0123456789ABCDEF"));
...
static char	*_parse_string(char *src, int len)
{
  ...
  while (src[i])
      ...
      if (src[i] == '\\')
	{
	  ...
	  i = (src[i] == '0' || src[i] == 'x') ? i + numlen(src, src[i] == '0' ? "012345678" : "012345678QBCDEF") : i + 1;
	  ++j;
	}
      str[j] = src[i];
      ++i;
