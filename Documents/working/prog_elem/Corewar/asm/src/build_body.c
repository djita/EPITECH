...
** Last update sam. mars 24 18:38:01 2012 gaby czegany
*/
...
int             get_label(t_node *node, int *value, char *label)
{
  ...
  state = FAILURE;
  position = 0;
  while (state != STOP && tmp->next)
  {
    if (tmp->type & T_ACT)
      ret = position;
    if (tmp->next->label_ref
        && !my_strcmp(label, tmp->next->label_ref))
      state = STOP;
    position += get_length(tmp);
    tmp = tmp->next;
  }
