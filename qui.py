...
## qui.py
##
## script python 2.x/3.x qui liste vos amis sur ns
...
##
## or
## $ python qui.py login_x

_list           = ["abiass_l::Laurence",
                   "bonmar_v::Vincent",
                   "cledat_g::Cledat",
                   "croize_m::Maxime",
                   "czegan_g::Gaby",
...
def get_comment(line):
  try:
    comment = _decode.sub(lambda code: binascii.unhexlify(code.group(0)[1:]),
                          line[8])
    return _cyan + (line[7] == "~" and line[7] or " ") + comment[:16] + _cend
  except UnicodeDecodeError:
    return _cyan + "#~~ERROR~~#" + _cend
